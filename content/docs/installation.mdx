...
<Callout type="info" title="PDF in JavaScript">
  ...
  pinned to the same liteparse version as the Rust engine. Chunking, metadata
  and the chunk schema are shared engine code and match exactly; the PDF→markdown
  step is the one place where they can differ, because it runs in the WASM
  package rather than in our engine. On our 22-document PDF corpus 19 match
  byte-for-byte, and 3 differ in a heading level or an emphasis span.
