...
For the "embed + persist" half, see the
[pgvector ingestion recipe](/docs/recipes#3-ingest-into-a-real-vector-database)
— it keys on `(source, chunk_index)` and upserts, which is what makes a task
retry safe. If documents change over time, the
[incremental re-indexing recipe](/docs/recipes#6-incremental-re-indexing) turns
this task into "re-embed only what moved".
